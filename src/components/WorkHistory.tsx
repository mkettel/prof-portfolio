  ...
  return (
    <div>
      {timeline.map((item, index) => (
        <div
          className="flex md:flex-row flex-col space-y-4 md:space-y-0 md:space-x-10 space-x-4 my-20 relative"
          key={`timeline-${index}`}
        >
          <Paragraph className="w-40 md:min-w-[150px]">{item.date}</Paragraph>
          <div>
            <Heading
              as="h5"
...
const Step = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="flex space-x-1 items-start my-2">
      <IconCircleCheckFilled className="h-3 w-4 min-w-4 min-h-3 mt-1 text-neutral-300" />
